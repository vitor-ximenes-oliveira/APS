...
def Dados(request, id):
        ...
        if "adicionar" in request.POST:
            try:
                ...
                messages.error(request,"Preencha todos os campos ou insira dados válidos")
        ...
        if "editar" in request.POST:
            try:
                ...
                editar_dados = Estacao.objects.filter(id=pk).update(dia=dia,hora=hora,estacao=estacao,codigo=codigo,poluente=poluente,
            ...
            pk = request.POST.get("excluir")
            apagarDado = Estacao.objects.filter(id=pk).delete()
