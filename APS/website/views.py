...
def Dados(request, id):
    ...
    if "adicionar" in request.POST:
        return redirect('inserirDado/'+str(id))
    elif "editar" in request.OST:
        return redirect('editarDado/'+str(id))
    elif "excluir" in request.OST:
        return redirect('excluirDado/'+str(id))

    return render(request,'Dados.html',{'dados':dados,'id':id})

def inserirDado(request,id):
        ...
        inserir_dados = Estacao.objects.create(dia=dia,hora=hora,estacao=estacao,codigo=codigo,poluente=poluente,
        valor=valor,unidade=unidade,tipo=tipo)
        messages.success(request,"Dado adicionado com sucesso")
        return redirect('../Dados/'+str(id))


def editarDado(request,id):
        dia = request.POST.get("dia")
        hora = request.POST.get("hora")
        estacao = request.POST.get("estacao")
        codigo = request.POST.get("codigo")
        ...
        unidade = request.POST.get("unidade")
        tipo = request.POST.get("tipo")
        inserir_dados = Estacao.objects.filter(id=id).update(dia=dia,hora=hora,estacao=estacao,codigo=codigo,poluente=poluente,
        valor=valor,unidade=unidade,tipo=tipo)
        messages.success(request,"Dado editado com sucesso")
        return redirect('../Dados/'+str(id)) 
     

def excluirDado(request,id):
